...
import type { NetworkStatuses } from "./akash";
...
// ─── Live-transport ↔ supply-chain mirror tools ────────────
// These connect observed vessel/aircraft positions to the supply-chain routes and
// facilities in the local Nexla mirror. Demo/replay path only appends to the local book;
// no cargo is ever inferred (positions are observations, not manifests).

export interface TransportIngestSummary {
  snapshotId: string;
  regionId: string;
  vessels: number;
  aircraft: number;
...
/** Register an observed transport snapshot with the Nexla mirror. */
export async function ingestTransportSnapshot(
  snapshot: LiveTransportSnapshot
): Promise<NexlaToolResult<TransportIngestSummary>> {
  const t0 = Date.now();
  const summary: TransportIngestSummary = {
    snapshotId: snapshot.id,
    regionId: snapshot.regionId,
    vessels: snapshot.vessels.length,
    aircraft: snapshot.aircraft.length,
    maritimeMode: snapshot.providers.maritime.mode,
    aviationMode: snapshot.providers.aviation.mode,
  };
  appendPosition({
    id: uid("tsig"),
    ts: nowIso(),
    kind: "signal",
    status: "pending",
    thesis: `Transport baseline ingested — ${summary.vessels} vessels, ${summary.aircraft} aircraft in ${summary.regionId} (maritime ${summary.maritimeMode}, aviation ${summary.aviationMode}).`,
    audit: { nexla_tool: "ingest_transport_snapshot", snapshot: snapshot.id },
  });
  return {
    ok: true,
    tool: "ingest_transport_snapshot",
    source: "nexla-local",
    data: summary,
    latencyMs: Date.now() - t0,
  };
...
export interface TransportRouteMapping {
  affectedRoutes: string[];
  vesselsLinked: number;
  aircraftLinked: number;
}

/** Associate observed assets with the disrupted routes/facilities in the world graph. */
export async function mapTransportAssetsToRoutes(
  snapshot: LiveTransportSnapshot,
  statuses: NetworkStatuses
): Promise<NexlaToolResult<TransportRouteMapping>> {
  const t0 = Date.now();
  const affectedRoutes = Object.entries(statuses.edgeStatuses)
    .filter(([, s]) => s !== "normal")
    .map(([id]) => id);
  const seaRoutes = affectedRoutes.length;
  const data: TransportRouteMapping = {
    affectedRoutes,
    // Deterministic, coarse linkage: how many observed assets could traverse the
    // affected routes (bounded by observed count). No manifest/cargo claims.
    vesselsLinked: seaRoutes > 0 ? snapshot.vessels.length : 0,
    aircraftLinked: seaRoutes > 0 ? snapshot.aircraft.length : 0,
  };
  return {
    ok: true,
    tool: "map_transport_assets_to_routes",
    source: "nexla-local",
    data,
    latencyMs: Date.now() - t0,
  };
...
/** Persist the immutable transport baseline record to the Nexla mirror. */
export async function recordTransportBaseline(
  snapshot: LiveTransportSnapshot
): Promise<NexlaToolResult<PositionEntry>> {
  const t0 = Date.now();
  const entry: PositionEntry = {
    ...
    kind: "thesis",
    status: "pending",
    thesis: `Transport baseline recorded for ${snapshot.regionId}: ${snapshot.vessels.length} vessels + ${snapshot.aircraft.length} aircraft observed at ${snapshot.capturedAt}.`,
    audit: { nexla_tool: "record_transport_baseline", snapshot: snapshot.id },
  };
  appendPosition(entry);
  return {
    ok: true,
    tool: "record_transport_baseline",
    source: "nexla-local",
    data: entry,
    latencyMs: Date.now() - t0,
  };
